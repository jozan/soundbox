...
var CBinWriter = function () {
  ...
  this.putUSHORT = function (x) {
    mData += String.fromCharCode(x & 255, (x >> 8) & 255);
  };
...
var CAudioTimer = function () {
  ...
  this.currentTime = function () {
    ...
    if (audioCurrentTime < 0.01 || err > 0.2 || err < -0.2) {
      ...
      for (var i = 0; i < mErrHist.length; i++) mErrHist[i] = 0;
    }

    // Error compensation (this should fix the problem when we're constantly
    // slightly off)
    var comp = 0;
    for (var i = 0; i < mErrHist.length; i++) comp += mErrHist[i];
...
var CTableEditor = function () {
  ...
  this.isSelected = function (col, row) {
    return (
      col >= self.selectionLeft() &&
      col <= self.selectionRight() &&
      row >= self.selectionTop() &&
      row <= self.selectionBottom()
    );
  };
  ...
  this.forAll = function (callback) {
    for (var row = 0; row < self.numrows(); ++row)
      for (var col = 0; col < self.numcols(); ++col) callback(col, row);
  };
  ...
  this.repeatSelection = function (repeatPeriod) {
    ...
    self.forSelection(function (col, row) {
      self.set(
        ...
        self.get(
          col,
          self.selectionTop() + ((row - self.selectionTop()) % repeatPeriod)
        )
      );
    });
    self.resumeUpdate();
  ...
  this.setCursor = function (col, row, keepSelection) {
    mCol = col;
    mRow = row;
    if (!keepSelection) self.deselect();
    self.update();
  ...
  this.update = function () {
    ...
    self.forAll(function (col, row) {
      ...
      if (o.innerHTML != desiredHTML) o.innerHTML = desiredHTML;
      var desiredClassName =
        mCol == col && mRow == row
          ? "cursor"
          : self.isSelected(col, row)
          ? "selected"
          : "";
      if (o.className != desiredClassName) o.className = desiredClassName;
    });
  };
  ...
  this.paste = function () {
    if (!mCopyBuffer) return;
    self.suppressUpdate();
    for (
      var row = self.row(), i = 0;
      row < self.numrows() && i < mCopyBuffer.length;
      ++row, ++i
    )
      for (
        var col = self.col(), j = 0;
        col < self.numcols() && j < mCopyBuffer[i].length;
        ++col, ++j
      )
...
var makeFourCC = function (fourChars) {
  return (
    (fourChars.charCodeAt(3) << 24) |
    (fourChars.charCodeAt(2) << 16) |
    (fourChars.charCodeAt(1) << 8) |
    fourChars.charCodeAt(0)
  );
};
...
var CGUI = function () {
  ...
  var mBlackKeyPos = [
    26, 1, 63, 3, 116, 6, 150, 8, 184, 10, 238, 13, 274, 15, 327, 18, 362, 20,
    394, 22,
  ];
  ...
  mSeq.numcols = function () {
    return MAX_CHANNELS;
  };

  mSeq.numrows = function () {
    return MAX_SONG_ROWS;
  };

  mSeq.toHTML = function (value) {
    if (value > 0)
      return (
        "" + (value <= 10 ? value - 1 : String.fromCharCode(64 + value - 10))
      );
    else return "&nbsp;";
  };
  ...
  mPattern.numcols = function () {
    return 4;
  };

  mPattern.numrows = function () {
    return mSong.patternLen;
  };

  mPattern.toHTML = function (value) {
    var n = value - 87;
    if (n > 0) return mNoteNames[n % 12] + Math.floor(n / 12);
    else return "&nbsp;";
  };
  ...
  mFxTrack.numcols = function () {
    return 1;
  };

  mFxTrack.numrows = function () {
    return mSong.patternLen;
  };
  ...
  var getURLSongData = function (dataParam) {
    ...
    if (dataParam) {
      ...
      if (str.indexOf("data:") == 0) {
        ...
        if (idx > 0) str2 = str.substr(idx + 7);
      } else {
  ...
  var makeEmptyChannel = function (patternLen) {
    ...
    // Sequence
    instr.p = [];
    for (j = 0; j < MAX_SONG_ROWS; j++) instr.p[j] = 0;
    ...
    for (j = 0; j < MAX_PATTERNS; j++) {
      col = {};
      col.n = [];
      for (k = 0; k < patternLen * 4; k++) col.n[k] = 0;
      col.f = [];
      for (k = 0; k < patternLen * 2; k++) col.f[k] = 0;
      instr.c[j] = col;
    }
  ...
  var compress = function (unpackedData) {
    ...
    if (compressionMethod == 0) {
      ...
      if (unpackedData === testData) compressionMethod = 1;
      else packedData = unpackedData;
    }
    // method:
    //  0: none
    //  1: RLE
  ...
  var songToBin = function (song) {
    ...
    for (i = 0; i < song.numChannels; i++) {
      ...
      // Patterns
      for (j = 0; j <= song.endPattern; j++) bin.putUBYTE(instr.p[j]);

      // Columns
      for (j = 0; j < MAX_PATTERNS; j++) {
        col = instr.c[j];
        for (k = 0; k < song.patternLen * 4; k++) bin.putUBYTE(col.n[k]);
        for (k = 0; k < song.patternLen * 2; k++) bin.putUBYTE(col.f[k]);
      }
    }
  ...
  var soundboxBinToSong = function (d) {
    ...
    // Last pattern to play
    if (version >= 12) song.endPattern = bin.getUSHORT();
    else song.endPattern = bin.getUBYTE() + 2;

    // Number of rows per pattern
    if (version >= 10) song.patternLen = bin.getUBYTE();
    else song.patternLen = 32;

    // Number of channels
    if (version >= 12) song.numChannels = bin.getUBYTE();
    else song.numChannels = 8;
    ...
    for (i = 0; i < song.numChannels; i++) {
      ...
      if (version < 6) {
        ...
        instr.i[OSC1_SEMI] = bin.getUBYTE();
        instr.i[OSC1_XENV] =
          version < 13 ? 64 * bin.getUBYTE() : bin.getUBYTE();
      }
      ...
      if (version < 6) {
        ...
        else instr.i[FX_FREQ] = bin.getUBYTE();
      ...
      // Patterns
      var song_rows;
      if (version < 9) song_rows = 48;
      else if (version < 12) song_rows = 128;
      else song_rows = song.endPattern + 1;
      instr.p = [];
      for (j = 0; j < song_rows; j++) instr.p[j] = bin.getUBYTE();
      for (j = song_rows; j < MAX_SONG_ROWS; j++) instr.p[j] = 0;
      ...
      for (j = 0; j < num_patterns; j++) {
        ...
        if (version == 1) {
          ...
          for (k = 0; k < song.patternLen * 4; k++) col.n[k] = bin.getUBYTE();
        }
        col.f = [];
        if (version < 4) {
          for (k = 0; k < song.patternLen * 2; k++) col.f[k] = 0;
        } else {
          for (k = 0; k < song.patternLen; k++) {
            var fxCmd = bin.getUBYTE();
            // We inserted two new commands in version 11
            if (version < 11 && fxCmd >= 14) fxCmd += 2;
            // We inserted ENV_EXP_DECAY in version 14
            if (version < 14 && fxCmd >= 14) fxCmd += 1;
            col.f[k] = fxCmd;
          }
          for (k = 0; k < song.patternLen; k++)
      ...
      for (j = num_patterns; j < MAX_PATTERNS; j++) {
        col = {};
        col.n = [];
        for (k = 0; k < song.patternLen * 4; k++) col.n[k] = 0;
        col.f = [];
        for (k = 0; k < song.patternLen * 2; k++) col.f[k] = 0;
        instr.c[j] = col;
      }
      ...
      if (version < 3) {
        ...
        instr.i[FX_DRIVE] =
          instr.i[FX_DRIVE] < 224 ? instr.i[FX_DRIVE] + 32 : 255;
      }
      if (version < 7) instr.i[FX_RESONANCE] = 255 - instr.i[FX_RESONANCE];
  ...
  var sonantBinToSong = function (d) {
    ...
    if (d.length != 3333) return undefined;
    if ((d.charCodeAt(3332) & 255) > 48) return undefined;
    ...
    for (i = 0; i < 8; i++) {
      ...
      // Patterns
      instr.p = [];
      for (j = 0; j < 48; j++) instr.p[j] = bin.getUBYTE();
      for (j = 48; j < MAX_SONG_ROWS; j++) instr.p[j] = 0;
      ...
      for (j = 0; j < 10; j++) {
        ...
        col.f = [];
        for (k = 0; k < 32 * 2; k++) col.f[k] = 0;
        instr.c[j] = col;
      }
      for (j = 10; j < MAX_PATTERNS; j++) {
        col = {};
        col.n = [];
        for (k = 0; k < 32 * 4; k++) col.n[k] = 0;
        col.f = [];
        for (k = 0; k < 32 * 2; k++) col.f[k] = 0;
        instr.c[j] = col;
      }
      ...
      if (instr.i[OSC1_WAVEFORM] == 2) instr.i[OSC1_VOL] /= 2;
      if (instr.i[OSC2_WAVEFORM] == 2) instr.i[OSC2_VOL] /= 2;
      if (instr.i[LFO_WAVEFORM] == 2) instr.i[LFO_AMT] /= 2;
  ...
  var binToSong = function (d) {
    ...
    if (!song) song = sonantBinToSong(d);
  ...
  var songToJS = function (song) {
    ...
    for (i = 0; i < song.numChannels; i++) {
      ...
      for (j = 0; j <= lastRow; j++) {
        ...
        if (pattern) lastNonZero = j;
      }
      for (j = 0; j <= lastNonZero; j++) {
        var pattern = instr.p[j];
        if (pattern) jsData += pattern;
        if (j < lastNonZero) jsData += ",";
      }
      jsData += "],\n";
      ...
      for (j = 0; j < maxPattern; j++) {
        ...
        for (k = 0; k < song.patternLen * 4; k++) {
          if (instr.c[j].n[k]) lastNonZero = k;
        }
        for (k = 0; k <= lastNonZero; k++) {
          var note = instr.c[j].n[k];
          if (note) jsData += note;
          if (k < lastNonZero) jsData += ",";
        }
        jsData += "],\n";
        jsData += "             f: [";
        lastNonZero = 0;
        for (k = 0; k < song.patternLen * 2; k++) {
          if (instr.c[j].f[k]) lastNonZero = k;
        }
        for (k = 0; k <= lastNonZero; k++) {
        ...
        jsData += "]}";
        if (j < maxPattern - 1) jsData += ",";
        jsData += "\n";
      }
      jsData += "          ]\n";
      jsData += "        }";
      if (i < song.numChannels) jsData += ",";
      jsData += "\n";
    }
    ...
    jsData +=
      "      patternLen: " + song.patternLen + ",  // Rows per pattern\n";
    jsData += "      endPattern: " + song.endPattern + ",  // End pattern\n";
    jsData +=
      "      numChannels: " + song.numChannels + "  // Number of channels\n";
    jsData += "    };\n";
  ...
  var onMIDIStarted = function (midi) {
    ...
    if (list.length) {
      for (var i = 0; i < list.length; i++) {
        mSelectMIDI.options[i] = new Option(
          list[i].name,
          list[i].fingerprint,
          i == preferredIndex,
          i == preferredIndex
  ...
  var setEditMode = function (mode) {
    ...
    document.getElementById("sequencer").className =
      mEditMode == EDIT_SEQUENCE ? "edit" : "";
    document.getElementById("pattern").className =
      mEditMode == EDIT_PATTERN ? "edit" : "";
    document.getElementById("fxtrack").className =
      mEditMode == EDIT_FXTRACK ? "edit" : "";
  ...
  var toHex = function (num, count) {
    ...
    for (var i = 0; i < leadingZeros; ++i) s = "0" + s;
    return s;
  };
  ...
  var playNote = function (n) {
    ...
    if (mEditMode == EDIT_PATTERN) {
      ...
      if (pat >= 0) {
        ...
        mPattern.setCursor(
          mPattern.col(),
          (mPattern.row() + 1) % mSong.patternLen
        );
        return true;
      }
    }
  ...
  var updateSlider = function (o, x) {
    ...
    if (valuetips.length > 0) valuetips[0].innerHTML = x;
  };
  ...
  var updateInstrument = function (resetPreset) {
    ...
    if (mEditMode == EDIT_FXTRACK) {
      ...
      if (fxCmd[0] > 0) instrI[fxCmd[0] - 1] = fxCmd[1];
    }

    // Oscillator 1
    document.getElementById("osc1_wave_sin").src =
      instrI[OSC1_WAVEFORM] == 0 ? "gui/wave-sin-sel.png" : "gui/wave-sin.png";
    document.getElementById("osc1_wave_sqr").src =
      instrI[OSC1_WAVEFORM] == 1 ? "gui/wave-sqr-sel.png" : "gui/wave-sqr.png";
    document.getElementById("osc1_wave_saw").src =
      instrI[OSC1_WAVEFORM] == 2 ? "gui/wave-saw-sel.png" : "gui/wave-saw.png";
    document.getElementById("osc1_wave_tri").src =
      instrI[OSC1_WAVEFORM] == 3 ? "gui/wave-tri-sel.png" : "gui/wave-tri.png";
    ...
    // Oscillator 2
    document.getElementById("osc2_wave_sin").src =
      instrI[OSC2_WAVEFORM] == 0 ? "gui/wave-sin-sel.png" : "gui/wave-sin.png";
    document.getElementById("osc2_wave_sqr").src =
      instrI[OSC2_WAVEFORM] == 1 ? "gui/wave-sqr-sel.png" : "gui/wave-sqr.png";
    document.getElementById("osc2_wave_saw").src =
      instrI[OSC2_WAVEFORM] == 2 ? "gui/wave-saw-sel.png" : "gui/wave-saw.png";
    document.getElementById("osc2_wave_tri").src =
      instrI[OSC2_WAVEFORM] == 3 ? "gui/wave-tri-sel.png" : "gui/wave-tri.png";
    ...
    // LFO
    document.getElementById("lfo_wave_sin").src =
      instrI[LFO_WAVEFORM] == 0 ? "gui/wave-sin-sel.png" : "gui/wave-sin.png";
    document.getElementById("lfo_wave_sqr").src =
      instrI[LFO_WAVEFORM] == 1 ? "gui/wave-sqr-sel.png" : "gui/wave-sqr.png";
    document.getElementById("lfo_wave_saw").src =
      instrI[LFO_WAVEFORM] == 2 ? "gui/wave-saw-sel.png" : "gui/wave-saw.png";
    document.getElementById("lfo_wave_tri").src =
      instrI[LFO_WAVEFORM] == 3 ? "gui/wave-tri-sel.png" : "gui/wave-tri.png";
    ...
    // Effects
    document.getElementById("fx_filt_lp").src =
      instrI[FX_FILTER] == 2 ? "gui/filt-lp-sel.png" : "gui/filt-lp.png";
    document.getElementById("fx_filt_hp").src =
      instrI[FX_FILTER] == 1 ? "gui/filt-hp-sel.png" : "gui/filt-hp.png";
    document.getElementById("fx_filt_bp").src =
      instrI[FX_FILTER] == 3 ? "gui/filt-bp-sel.png" : "gui/filt-bp.png";
  ...
  var setPatternLength = function (length) {
    if (mSong.patternLen === length) return;
    ...
    for (i = 0; i < MAX_CHANNELS; i++) {
      for (j = 0; j < MAX_PATTERNS; j++) {
        ...
        notes = [];
        fx = [];
        for (k = 0; k < 4 * length; k++) notes[k] = 0;
        for (k = 0; k < 2 * length; k++) fx[k] = 0;
  ...
  var loadSongFromData = function (songData) {
    ...
    if (song) {
      ...
      if (!deepEquals(mSong, mSongUnmodified)) {
        var ok = confirm(
          "Load the song? Unsaved changes to your current song will be lost."
        );
        if (!ok) return false;
      }
      stopAudio();
  ...
  var showOpenDialog = function () {
    ...
    o.appendChild(
      document.createTextNode(
        "Hint: You can also drag'n'drop binary files into the editor."
      )
    );
    form.appendChild(o);
          ...
          if (e.checked) {
            for (var j = 0; j < gDemoSongs.length; j++) {
              if (gDemoSongs[j].name === e.value) {
                if (gDemoSongs[j].data) songData = gDemoSongs[j].data;
                else songData = getURLSongData(gDemoSongs[j].base64);
                break;
              }
            }
            break;
  ...
  var showSaveDialog = function () {
    ...
    o = document.createElement("p");
    o.appendChild(
      document.createTextNode("Data URL (copy/paste, bookmark, mail etc):")
    );
    parent.appendChild(o);
    ...
    o.onclick = function () {
      var dataURI =
        "data:application/octet-stream;base64," + btoa(songToBin(mSong));
      window.open(dataURI);
      hideDialog();
      return false;
    };
    ...
    var formCloud = document.createElement("form");
    var inputCloud = document.createElement("input");
    var inputName = document.createElement("input");
    var cloudSaveBtn = document.createElement("button");

    inputCloud.type = "text";
    inputCloud.value = btoa(songToBin(mSong));
    inputName.type = "text";
    inputName.placeholder = "Track name";
    inputName.required = true;
    inputName.id = "inputName";
    inputName.name = "inputName";

    cloudSaveBtn.type = "submit";
    cloudSaveBtn.innerHTML = "Save to Cloud";

    formCloud.addEventListener("submit", (e) => {
      e.preventDefault();
      async function run() {
        const data = inputCloud.value;
    ...
    formCloud.appendChild(inputName);
    formCloud.appendChild(inputCloud);
    formCloud.appendChild(cloudSaveBtn);

    parent.appendChild(formCloud);
  ...
  var showAboutDialog = function () {
    ...
    o = document.createElement("p");
    o.innerHTML =
      'Licensed under the <a href="gpl.txt" target="_blank" rel="noopener noreferrer">GPL v3</a> license ' +
  ...
  var newSong = function (e) {
    ...
    if (!deepEquals(mSong, mSongUnmodified)) {
      var ok = confirm(
        "Start a new song? Unsaved changes to your current song will be lost."
      );
      if (!ok) return false;
    }
  ...
  var generateAudio = function (doneFun, opts) {
    ...
    mPlayer.generate(mSong, opts, function (progress) {
      ...
      if (progress >= 1) {
        ...
        var d2 = new Date();
        setStatus(
          "Generation time: " + (d2.getTime() - d1.getTime()) / 1000 + "s"
        );
  ...
  var windowBeforeUnload = function (e) {
    ...
    if (!deepEquals(mSong, mSongUnmodified)) {
      var confirmationMessage =
        "Warning: song has unsaved changes. If you leave before saving, your changes will be lost.";
  ...
  var getSamplesSinceNote = function (t, chan) {
    ...
    for (var k = 0; k < mSong.patternLen; ++k) {
      ...
      for (var patCol = 0; patCol < 4; patCol++) {
        if (
          pat >= 0 &&
          mSong.songData[chan].c[pat].n[patPos + patCol * mSong.patternLen] > 0
        )
  ...
  var redrawPlayerGfx = function (t) {
    ...
    if (ctx) {
      ...
      ctx.lineTo(
        w * 0.25 + h * 1.8 * Math.sin(a1),
        h * 2.1 - h * 1.8 * Math.cos(a1)
      );
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(w * 0.75, h * 1.45);
      ctx.lineTo(
        w * 0.75 + h * 1.8 * Math.sin(a2),
        h * 2.1 - h * 1.8 * Math.cos(a2)
      );
      ctx.stroke();
      ...
      for (i = 0; i < MAX_CHANNELS; ++i) {
        ...
        if (i >= mFollowerFirstCol && i <= mFollowerLastCol) {
          ...
          if (numSamp >= 0 && numSamp < env_tot) {
            ...
            var alpha;
            if (numSamp < env_a) alpha = numSamp / env_a;
            else alpha = 1 - (numSamp - env_a) / env_r;
  ...
  var playGeneratedWave = function (wave) {
    ...
    try {
      if (mAudio) {
        ...
        mAudio.src = URL.createObjectURL(
        ...
        mAudioContext.decodeAudioData(
          ...
          function (buffer) {
            try {
              ...
              if (mAudioSourceNode.start) mAudioSourceNode.start(0);
              else mAudioSourceNode.noteOn(0);
  ...
  var instrCopyClick = function (e) {
    ...
    for (var i = 0; i <= instr.i.length; ++i) mInstrCopyBuffer[i] = instr.i[i];
  };
  ...
  var instrSaveClick = function (e) {
    ...
    var dataURI =
      "data:application/octet-stream;base64," + btoa(instrumentToBin(instrI));
    window.open(dataURI);
  };
  ...
  var boxMouseDown = function (e) {
    ...
    var fxCmd = -1;
    if (o.id === "lfo_fxfreq") fxCmd = LFO_FX_FREQ;
  ...
  var fxFiltMouseDown = function (e) {
    ...
    if (mEditMode == EDIT_FXTRACK) mFxTrack.setCurrent([FX_FILTER + 1, filt]);
  ...
  var octaveUp = function (e) {
    ...
    if (mKeyboardOctave < 8) {
      mKeyboardOctave++;
      document.getElementById("keyboardOctave").innerHTML =
        "" + mKeyboardOctave;
    }
  };
  ...
  var keyboardMouseDown = function (e) {
    ...
    if (p[1] < 68) {
      ...
      for (var i = 0; i < mBlackKeyPos.length; i += 2) {
        if (p[0] >= mBlackKeyPos[i] - 10 && p[0] <= mBlackKeyPos[i] + 10) {
          n = mBlackKeyPos[i + 1];
          break;
        }
      }
  ...
  var sliderMouseDown = function (e) {
    ...
    e.preventDefault();
    var valuetips =
      mActiveSlider.parentElement.getElementsByClassName("valuetip");
    if (valuetips.length > 0) valuetips[0].style.visibility = "visible";
  };
  ...
  var mouseMove = function (e) {
    ...
    if (mActiveSlider) {
      ...
      if (mActiveSlider.sliderProps.nonLinear) x = x * x;
      ...
      if (cmdNo === ARP_CHORD) {
        ...
        else x = (instr.i[ARP_CHORD] & 240) | x;
      }
  ...
  var keyDown = function (e) {
    ...
    var tableEditor =
      mEditMode == EDIT_SEQUENCE
        ? mSeq
        : mEditMode == EDIT_PATTERN
        ? mPattern
        : mEditMode == EDIT_FXTRACK
        ? mFxTrack
        : undefined;
    ...
    if (mEditMode == EDIT_SEQUENCE) {
      ...
      // 0 - 9
      if (e.keyCode >= 48 && e.keyCode <= 57) patternCode = e.keyCode - 47;

      // A - Z
      if (e.keyCode >= 64 && e.keyCode <= 90) patternCode = e.keyCode - 54;
    ...
    if (tableEditor) {
      ...
      switch (e.keyCode) {
        case 39: // RIGHT
          if (e.ctrlKey) col = tableEditor.numcols() - 1;
          else col = col + 1;
          break;
        case 37: // LEFT
          if (e.ctrlKey) col = 0;
          ...
          break;
        case 40: // DOWN
          if (e.ctrlKey) row = tableEditor.numrows() - 1;
          else row = row + 1;
          break;
        case 38: // UP
          if (e.ctrlKey) row = 0;
        ...
        case 46: // DELETE
          ...
          tableEditor.modifySelection(function (value) {
            return emptyCell;
          });
          if (tableEditor.isSingleSelected()) row = row + 1;
          break;
      }
      if (oldRow != row || oldCol != col) {
        ...
        if (e.shiftKey) {
          if (e.altKey) tableEditor.setSelectionCorner2(col, row);
          else tableEditor.setSelectionCorner(col, row);
        } else {
    ...
    switch (e.keyCode) {
      ...
      case 13: // ENTER / RETURN
        if (editingBpmRpp) {
          ...
          document.getElementById("bpm").blur();
          document.getElementById("rpp").blur();
        } else if (mEditMode == EDIT_SEQUENCE) fillSequenceRange();
        else if (mEditMode == EDIT_FXTRACK) fillFxRange();
        else if (mEditMode == EDIT_PATTERN) fillPatternRange();
        break;
    }
  ...
  var fillFxRange = function () {
    if (mFxTrack.selectionBottom() <= mFxTrack.selectionTop() + 1) return;
    var cmd1 = mFxTrack.get(0, mFxTrack.selectionTop())[0];
    var cmd2 = mFxTrack.get(0, mFxTrack.selectionBottom())[0];
    if (cmd1 != cmd2) return;
    for (
      var row = mFxTrack.selectionTop() + 1;
      row < mFxTrack.selectionBottom();
      ++row
    )
      if (mFxTrack.get(0, row)[0]) return;
    ...
    for (
      ...
      var alpha =
        ((row - mFxTrack.selectionTop()) /
          (mFxTrack.selectionBottom() - mFxTrack.selectionTop())) *
        1.0;
      var roundedValue = (alpha * val2 + (1 - alpha) * val1 + 0.5) | 0;
  ...
  var fillPatternRange = function () {
    if (mPattern.selectionBottom() <= mPattern.selectionTop()) return;
    var patternLen = mSong.patternLen;
    // find the last row that is not all empty
    outer: for (
      var lastRow = mPattern.selectionBottom();
      lastRow > mPattern.selectionTop();
      --lastRow
    ) {
      for (
        var col = mPattern.selectionLeft();
        col <= mPattern.selectionRight();
        ++col
      )
        if (mPattern.get(col, lastRow)) break outer;
    }
  ...
  var onFileDrop = function (e) {
    ...
    reader.onload = function (e) {
      ...
      if (!success)
        // if it is not a song, maybe its an instrument
        success = loadInstrumentFromData(data);
      if (!success) alert("Binary format not recognized.");
    };
  ...
  var getCurrentBeatDistance = function (table) {
    ...
    while (beatDistance < table.children.length) {
      if (table.children[beatDistance].className === "beat") break;
      beatDistance++;
    }
    return beatDistance;
  };
  ...
  var getBeatDistance = function () {
    var bpm = getBPM();
    var beatDistance = 4;
    if (mSong.patternLen % 3 === 0) beatDistance = 3;
    else if (mSong.patternLen % 4 === 0) beatDistance = 4;
    else if (mSong.patternLen % 2 === 0) beatDistance = 2;
    else if (mSong.patternLen % 5 === 0) beatDistance = 5;
    if (
      bpm / beatDistance >= 40 &&
      mSong.patternLen > 24 &&
      mSong.patternLen % (beatDistance * 2) === 0
    )
      beatDistance *= 2;
  ...
  var buildPatternTable = function () {
    ...
    if (
      table.children.length === mSong.patternLen &&
      getCurrentBeatDistance(table) === beatDistance
    )
      return;
    ...
    for (var row = 0; row < mSong.patternLen; row++) {
      tr = document.createElement("tr");
      if (row % beatDistance === 0) tr.className = "beat";
      th = document.createElement("th");
      th.id = "ppr" + row;
      th.textContent = "" + row;
      tr.appendChild(th);
  ...
  var unlockAudioPlaybackHack = function () {
    ...
    if (mAudioContext) {
      ...
      if (dummySource.start) dummySource.start(0);
      else dummySource.noteOn(0);
    }
  ...
  this.init = function () {
    ...
    // Load the song
    var songData = getURLSongData(
      mGETParams && mGETParams.data && mGETParams.data[0]
    );
    ...
    document.getElementById("sequencerCopy").onmousedown = preventDefault(
      mSeq.copy
    );
    document.getElementById("sequencerPaste").onmousedown = preventDefault(
      mSeq.paste
    );
    document.getElementById("sequencerPatUp").onmousedown = preventDefault(
      function (e) {
    ...
    document.getElementById("sequencerPatDown").onmousedown = preventDefault(
      function (e) {
        mSeq.modifySelection(function (value) {
    ...
    document.getElementById("patternCopy").onmousedown = preventDefault(
      mPattern.copy
    );
    document.getElementById("patternPaste").onmousedown = preventDefault(
      mPattern.paste
    );
    document.getElementById("patternNoteUp").onmousedown = preventDefault(
      function (e) {
    ...
    document.getElementById("patternNoteDown").onmousedown = preventDefault(
      function (e) {
        mPattern.modifySelection(function (value) {
    ...
    document.getElementById("patternOctaveUp").onmousedown = preventDefault(
      function (e) {
        mPattern.modifySelection(function (value) {
    ...
    document.getElementById("patternOctaveDown").onmousedown = preventDefault(
      function (e) {
        mPattern.modifySelection(function (value) {
    ...
    document.getElementById("fxCopy").onmousedown = preventDefault(
      mFxTrack.copy
    );
    document.getElementById("fxPaste").onmousedown = preventDefault(
      mFxTrack.paste
    );
    ...
    document
      .getElementById("osc1_wave_sin")
      .addEventListener("mousedown", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_sin")
      .addEventListener("touchstart", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_sqr")
      .addEventListener("mousedown", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_sqr")
      .addEventListener("touchstart", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_saw")
      .addEventListener("mousedown", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_saw")
      .addEventListener("touchstart", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_tri")
      .addEventListener("mousedown", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_wave_tri")
      .addEventListener("touchstart", osc1WaveMouseDown, false);
    document
      .getElementById("osc1_vol")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc1_vol")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("osc1_semi")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc1_semi")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("osc1_xenv")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc1_xenv")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("osc2_wave_sin")
      .addEventListener("mousedown", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_sin")
      .addEventListener("touchstart", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_sqr")
      .addEventListener("mousedown", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_sqr")
      .addEventListener("touchstart", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_saw")
      .addEventListener("mousedown", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_saw")
      .addEventListener("touchstart", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_tri")
      .addEventListener("mousedown", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_wave_tri")
      .addEventListener("touchstart", osc2WaveMouseDown, false);
    document
      .getElementById("osc2_vol")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc2_vol")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("osc2_semi")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc2_semi")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("osc2_det")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc2_det")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("osc2_xenv")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("osc2_xenv")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("noise_vol")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("noise_vol")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("env_att")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("env_att")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("env_sust")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("env_sust")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("env_rel")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("env_rel")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("env_decay")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("env_decay")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("arp_note1")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("arp_note1")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("arp_note2")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("arp_note2")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("arp_speed")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("arp_speed")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("lfo_wave_sin")
      .addEventListener("mousedown", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_sin")
      .addEventListener("touchstart", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_sqr")
      .addEventListener("mousedown", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_sqr")
      .addEventListener("touchstart", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_saw")
      .addEventListener("mousedown", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_saw")
      .addEventListener("touchstart", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_tri")
      .addEventListener("mousedown", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_wave_tri")
      .addEventListener("touchstart", lfoWaveMouseDown, false);
    document
      .getElementById("lfo_amt")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("lfo_amt")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("lfo_freq")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("lfo_freq")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("lfo_fxfreq")
      .addEventListener("mousedown", boxMouseDown, false);
    document
      .getElementById("lfo_fxfreq")
      .addEventListener("touchstart", boxMouseDown, false);
    document
      .getElementById("fx_filt_lp")
      .addEventListener("mousedown", fxFiltMouseDown, false);
    document
      .getElementById("fx_filt_lp")
      .addEventListener("touchstart", fxFiltMouseDown, false);
    document
      .getElementById("fx_filt_hp")
      .addEventListener("mousedown", fxFiltMouseDown, false);
    document
      .getElementById("fx_filt_hp")
      .addEventListener("touchstart", fxFiltMouseDown, false);
    document
      .getElementById("fx_filt_bp")
      .addEventListener("mousedown", fxFiltMouseDown, false);
    document
      .getElementById("fx_filt_bp")
      .addEventListener("touchstart", fxFiltMouseDown, false);
    document
      .getElementById("fx_freq")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_freq")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_res")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_res")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_dly_amt")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_dly_amt")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_dly_time")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_dly_time")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_pan_amt")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_pan_amt")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_pan_freq")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_pan_freq")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_dist")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_dist")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      .getElementById("fx_drive")
      .addEventListener("mousedown", sliderMouseDown, false);
    document
      .getElementById("fx_drive")
      .addEventListener("touchstart", sliderMouseDown, false);
    document
      ...
      .addEventListener("mousedown", octaveDown, false);
    document
      .getElementById("octaveDown")
      .addEventListener("touchstart", octaveDown, false);
    document
      .getElementById("octaveUp")
      .addEventListener("mousedown", octaveUp, false);
    document
      .getElementById("octaveUp")
      .addEventListener("touchstart", octaveUp, false);
    document
      .getElementById("keyboard")
      .addEventListener("mousedown", keyboardMouseDown, false);
    document
      .getElementById("keyboard")
      .addEventListener("touchstart", keyboardMouseDown, false);

    document
      .getElementById("instrCopy")
      .addEventListener("click", instrCopyClick, false);
    document
      .getElementById("instrPaste")
      .addEventListener("click", instrPasteClick, false);
    document
      .getElementById("instrSave")
      .addEventListener("click", instrSaveClick, false);
